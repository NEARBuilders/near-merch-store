...
export class OrderStore extends Context.Tag("OrderStore")<
  OrderStore,
  {
    ...
    readonly updateCheckout: (orderId: string, checkoutSessionId: string, checkoutProvider: 'near' | 'pingpay') => Effect.Effect<OrderWithItems, Error>;
  ...
  Effect.gen(function* () {
    ...
    const rowToOrder = async (row: typeof schema.orders.$inferSelect): Promise<OrderWithItems> => {
      ...
      return {
        ...
        checkoutProvider: row.checkoutProvider === 'near' || row.checkoutProvider === 'pingpay'
