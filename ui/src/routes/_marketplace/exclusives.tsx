...
function ExclusivesPage() {
  ...
  const matchesSearchQuery = (product: Product, query: string): boolean => {
    ...
    const searchText = `${product.title} ${product.productType || ''} ${product.brand || ''}`.toLowerCase();
  ...
  }, [allProductsData, searchQuery, collectionFilter, brandFilter, sizeFilter, colorFilter, priceRange, discountFilter, sortBy]);
          ...
          <div className="md:hidden flex items-center gap-2">
            <div className="flex-1 relative">
              ...
              <Input
                ...
                className="pl-10 h-11 bg-background/70 border border-border/60 rounded-lg text-sm focus-visible:ring-0 focus-visible:ring-offset-0 focus-visible:border-[#00EC97] hover:border-border/60"
              />
            </div>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <div className="text-muted-foreground mb-4">
                ...
                {searchQuery
                  ? `No exclusive products found matching "${searchQuery}".`
                  : "There are currently no exclusive products available."}
                  {isError && " The API may be temporarily unavailable."}
                </p>
              </div>
            {(searchQuery || priceRange !== "all" || discountFilter !== "all" || sizeFilter !== "all" || colorFilter !== "all" || brandFilter !== "all") && (
                <Button
                  variant="outline"
                onClick={() => {
                  setSearchQuery("");
