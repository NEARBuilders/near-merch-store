...
Use this workflow when you want repeatable, fast iteration on how the system behaves (DB state, API mutations, end-to-end flows, performance snapshots).

```bash
# 1) Install deps for the CLI package (not part of root workspace)
cd packages/cli
bun install

# 2) Run the Docker Postgres backed integration test
bunx vitest run tests/integration/marketplace-product-mutations.test.ts
```

This test:
- starts an isolated Postgres container
- applies the Drizzle SQL migration(s)
- seeds minimal marketplace data
- boots the API dev server and executes read/write flows
