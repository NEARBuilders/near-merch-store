...
Also used for:
- Running the repo's CLI-driven integration tests (process/port monitoring + Playwright)
- Debugging dev logs under `.bos/logs/`
- Iterating on performance regressions via repeatable, DB-backed flows
...
For the local DB-backed test loop, see [testing.md](docs/testing.md).
